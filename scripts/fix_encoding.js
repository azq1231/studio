...
const filePath = path.join(__dirname, '..', 'studio', 'src', 'components', 'finance-flow-client.tsx');
let content = fs.readFileSync(filePath, 'utf8');

// The mangled names we saw: '2357.TW': '?舐◣', '3231.TW': '蝺臬'
// We will replace the entire nameMap object string with a clean one.

const cleanNameMap = `  const nameMap: Record<string, string> = {
...
// Find the existing nameMap and replace it.
const regex = /const nameMap: Record<string, string> = \{[\s\S]*?\};/;
if (regex.test(content)) {
    content = content.replace(regex, cleanNameMap);
    fs.writeFileSync(filePath, content, 'utf8');
    console.log('✅ Successfully reframed nameMap with UTF-8 encoding.');
} else {
    console.error('❌ Could not find nameMap pattern in the file.');
    process.exit(1);
}

...
async function processSingleCreditEntry(entry: ParsedCreditDataWithCategory, replacementRules: ReplacementRule[], categoryRules: CategoryRule[]): Promise<CreditData | null> {
    ...
    // Re-generate ID after rules are applied to ensure it's based on the final, clean data.
    const finalIdString = `${entry.transactionDate}-${entry.postingDate}-${description}-${entry.amount}`;
    const finalId = await sha1(finalIdString);

    return {
        id: finalId, // Use the re-generated, deterministic ID
